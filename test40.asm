...
INIT:
        ...
        LD      (RHYTVOL), A    ; リズム音源のトータルボリューム初期化
        LD      C, 011H
        CALL    OUT45           ; OPNAに反映
...
PMDHK3:
        ...
        CP      10H             ; アドレスが一致しているか
        JP      NZ, HK3RT       ; レジスタCHが10H以外は処理せず抜けさせる
...
UNMUTE:
        ...
        LD      A, (SELCH)      ; リズムチャンネルの場合のみ音量復元
        CP      10
        JR      NZ, MUCHEND
        LD      A, (RHYTVOL)    ; 保存した値を復元
        LD      (0BCFH), A
        LD      C, 011H
        CALL    OUT45
        JP      MUCHEND
...
ADPCMMUTE:
        LD      (IX+VOLPUSH), -255 ; ADPCMの音量を最小に
        CALL    PCMMAIN         ; ミュート時も再生処理を呼び出して音名データを更新
        JP      RHYTHMS
...
RHYTVOL:DB      03CH            ; リズム音源のトータルボリューム (デフォルト3CH)
